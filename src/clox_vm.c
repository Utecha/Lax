...
static bool callValue(Value callee, int argCount)
{
    ...
    runtimeError("Can only call a class, function, or method.");
    return false;
}
...
static bool invoke(ObjString *name, int argCount)
{
    ...
    if (!IS_INSTANCE(receiver)) {
        runtimeError("Methods can only be defined on instances of a class.");
        return false;
    }
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_SET_PROPERTY: {
                if (!IS_INSTANCE(peek(1))) {
                    runtimeError("Fields can only be defined on instances of a class.");
                    return INTERPRET_RUNTIME_ERROR;
                }
...
{
    ObjFunction *function = bCompile(source);
